...
ReplicateResult btb_wl_model_CTMC(const Parameters& p)
{
  ...
  while(time < n_tsteps)
  {
    ...
    //deer-deer transmission (freq dependence)
    double interspecific_trans_rate = p.deer_deer_contact_rate * p.transmission_rate * (double(nS+nSuperS)) * ((double(nI+nSuperI)) / (double(N_d)));
    
    //transmission from infected farm contact (freq)
    double farm_trans_rate = farm_contact_rate_v[current_quarter_idx] * p.transmission_rate * ( (nS + p.SS_farm_contact_factor * nSuperS) / N_d );    
