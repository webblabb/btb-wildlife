...
if (section_id == 1) {
  ...
  write.csv(results, "data/ss_herdsize_meanprev_freq.csv", row.names = FALSE)
}
...
if (section_id == 2) {
  ...
  write.csv(results, "data/herdsize_initnum_fadeout_freq.csv", row.names = FALSE)
}
...
if (section_id == 3) {
  ...
  write.csv(results, "data/hunt_herdsize_freq.csv", row.names = FALSE)
}
...
if (section_id == 4) {
  ...
  write.csv(results, "data/beta_p2_hunt_freq.csv", row.names = FALSE)
}
...
if (section_id == 5) {
  ...
  par_file = paste0('bTBwl_LHSpars_',infType,"_freq")
  lambda_factor = 1.2
  save_pars = T
  save_runs = F
  save_plots = F
  ...
  if(def_seed == 43 & N_LHS_sets == 1000 & dim(lh)[1] == N_LHS_sets){
    ...
    write.csv(lh, paste0("data/",par_file,"_freq.csv"))
  }
  ...
  write.csv(R, file = paste0("./data/LHS_summary_", infType, "_freq.csv" ), row.names = F)
...
if (section_id == 6) {
  ...
  write.csv(results, "data/xi_phi_meanprev_freq.csv")
}
